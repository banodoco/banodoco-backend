...
from ai_project.v1.serializers.dao import CreateFileDao, FileListFilterDao, UUIDDao, UpdateFileDao
...
class FileAPIView(APIView):
    ...
    def get(self, request):
        ...
        file = InternalFileObject.objects.filter(uuid=attributes.data['uuid'], is_disabled=False).first()
        if not file:
            return success({}, 'invalid file uuid', False)
        
        if file.project:
            if file.project.user.uuid != request.role_id and request.role_id != UserType.ADMIN.value:
                return unauthorized({})
