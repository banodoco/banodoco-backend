...
from ai_project.v1.serializers.dao import CreateInferenceLogDao, InferenceLogListFilterDao, UUIDDao, UpdateInferenceLogDao
...
class InferenceLogView(APIView):
    ...
    def get(self, request):
        ...
        log = InferenceLog.objects.filter(uuid=attributes.data['uuid'], is_disabled=False).first()
        if not log:
